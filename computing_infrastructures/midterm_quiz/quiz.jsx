...
This is the same as RAID 5 — the difference between RAID 4 and 5 is WHERE parity is stored (dedicated disk vs. distributed), not HOW MANY operations are needed.`
  ...
  // ===== CATEGORY: Building Infrastructure =====
  {
    category: "Building Infrastructure",
    type: "theory",
    question: "What does PUE stand for and what does a PUE of 1.0 mean?",
    options: [
      "Power Usage Effectiveness; PUE 1.0 means half the power goes to IT equipment",
      "Power Unit Efficiency; PUE 1.0 means the data center is at maximum capacity",
      "Power Usage Effectiveness; PUE 1.0 means ALL power consumed is used by IT equipment (perfect efficiency)",
      "Processing Unit Energy; PUE 1.0 means each server uses exactly 1 kW"
    ],
    correct: 2,
    explanation: "PUE = Total Facility Power / IT Equipment Power. A PUE of 1.0 is the theoretical ideal: all power goes directly to computing. In reality, PUE > 1.0 because cooling, UPS, lighting, and other overhead consume additional power. Average data center PUE is ~1.8. Google's data centers achieve ~1.10. DCiE (Data Center infrastructure Efficiency) is the inverse: 1/PUE."
  },
  {
    category: "Building Infrastructure",
    type: "calculation",
    question: "A data center consumes 10 MW total and its IT equipment uses 6 MW. What is the PUE and DCiE?",
    options: [
      "PUE = 1.67, DCiE = 60%",
      "PUE = 0.60, DCiE = 167%",
      "PUE = 1.40, DCiE = 71%",
...
1. PUE = Total Facility Power / IT Equipment Power
   PUE = 10 MW / 6 MW = 1.667 ≈ 1.67

2. DCiE = 1 / PUE = IT Equipment Power / Total Facility Power
   DCiE = 6 / 10 = 0.60 = 60%

This means 60% of the total power goes to actual computing, while 40% is overhead (cooling, UPS, lighting, etc.). This PUE of 1.67 is roughly average for data centers.`
  },
  {
    category: "Building Infrastructure",
    type: "theory",
    question: "What does a UPS (Uninterruptible Power Supply) do in a data center?",
    options: [
      "Only provides battery backup during outages",
      "Combines three functions: power input switching (utility/generator), energy storage to bridge power transitions, and power conditioning",
      "Only conditions incoming power to remove voltage spikes",
      "Only switches between utility and generator power"
    ],
    correct: 1,
    explanation: "A UPS combines three critical functions: (1) Transfer switch that selects between utility power and generator power, (2) Energy storage (batteries, flywheels, etc.) to bridge the time between utility failure and generator startup, (3) Power conditioning to remove voltage spikes, sags, and harmonic distortions from the AC feed."
  },
  {
    category: "Building Infrastructure",
    type: "theory",
    question: "What is in-row cooling in a data center?",
    options: [
      "Cooling units placed on the roof of the data center",
      "Cooling units placed between server racks, allowing higher server densities and increased rack power densities",
      "Cooling each server individually with a dedicated fan",
      "Using outside air for cooling"
    ],
    correct: 1,
    explanation: "In-row cooling places cooling units directly between server racks within a row. This allows for much more targeted and efficient cooling compared to room-level cooling. The proximity of cooling to heat sources enables higher server densities and increased rack power densities."
  },
  {
    category: "Building Infrastructure",
    type: "theory",
    question: "What is the main purpose of a raised floor in a data center?",
    options: [
      "To protect against earthquakes",
      "To create a plenum (space) beneath the servers for cold air distribution, cable routing, and fire suppression",
      "To make the room look bigger",
      "To reduce noise from servers"
    ],
    correct: 1,
    explanation: "A raised floor creates an underfloor plenum used for: (1) Cold air distribution — air conditioning pushes cold air under the floor, which rises through perforated tiles to cool servers, (2) Cable routing — power and network cables can run beneath the floor, reducing clutter, (3) Fire suppression systems can also use this space. This design is fundamental to the hot-aisle/cold-aisle cooling approach."
  },
  {
    category: "Building Infrastructure",
    type: "theory",
    question: "What is liquid cooling in a data center context?",
    options: [
      "Flooding the server room with coolant",
      "Using cold plates (liquid-cooled heat sinks) to directly cool high-power components like GPUs/TPUs, while other components remain air-cooled",
      "Replacing all fans with water pumps",
      "Submerging entire racks in cooling fluid"
    ],
    correct: 1,
    explanation: "Liquid cooling uses cold plates (local liquid-cooled heat sinks) attached to components with the highest power dissipation (e.g., CPUs, GPUs, TPUs). It's impractical to cool all components this way, so typically the highest-power components get liquid cooling while others remain air-cooled. The liquid carries heat to an exchanger near the rack or as part of the building infrastructure."
  },
  ...
  // ===== CATEGORY: Networking =====
  {
    category: "Networking",
    type: "theory",
    question: "What is the difference between East-West and North-South traffic in a data center?",
    options: [
      "East-West is incoming traffic, North-South is outgoing traffic",
      "East-West is traffic between servers within the data center, North-South is traffic between the data center and external clients",
      "East-West is faster traffic, North-South is slower traffic",
      "They refer to geographic directions of data center locations"
    ],
    correct: 1,
    explanation: "East-West traffic flows laterally between servers within the same data center (e.g., storage replication, VM migration, distributed computation). North-South traffic flows vertically between the data center and external clients/internet. In modern data centers, East-West traffic is typically MUCH larger than North-South traffic — a 1-byte N-S transaction can generate on average 3-4 bytes of E-W traffic."
  },
  {
    category: "Networking",
    type: "theory",
    question: "True or False: In current datacenters, east-west traffic typically involves lower data volumes compared to north-south traffic.",
    options: ["True", "False"],
    correct: 1,
    explanation: "False. East-West traffic (server-to-server within the DC) is typically MUCH larger than North-South traffic (DC to external). According to Cisco's Global Cloud Index: in a data center, East-West traffic accounts for ~76% of total traffic, while North-South is ~17%. This is driven by storage replication, VM migration, and distributed processing."
  },
  {
    category: "Networking",
    type: "theory",
    question: "What is a leaf-spine (Clos) network topology?",
    options: [
      "A single large switch connecting all servers",
      "A two-tier architecture where every leaf switch connects to every spine switch, providing equal-cost paths between any two servers",
      "A ring topology where switches are connected in a circle",
      "A tree topology with a single root switch"
    ],
    correct: 1,
    explanation: "Leaf-spine is a two-tier topology derived from Clos networks. Every leaf (ToR) switch connects to every spine switch, and every path between any two leaf switches traverses exactly one spine switch (1 or 3 hops total). This provides equal bandwidth between any two servers, eliminates bottlenecks, and is ideal for East-West heavy traffic patterns in modern data centers."
  },
  {
    category: "Networking",
    type: "theory",
    question: "True or False: Leaf-spine topologies are not suitable for highly virtualized environments with many virtual machines.",
    options: ["True", "False"],
    correct: 1,
    explanation: "False. Leaf-spine topologies are actually IDEAL for highly virtualized environments. VM migration generates heavy East-West traffic, and leaf-spine provides equal-bandwidth paths between any two servers. The non-blocking, multi-path nature of leaf-spine ensures consistent performance even with heavy server-to-server traffic from VM operations."
  },
  {
    category: "Networking",
    type: "calculation",
    question: "In a homogeneous leaf-spine design with k×k switches (2k ports each), k servers per leaf, and k spine switches, how many total servers can be connected with k=32?",
    options: ["2048 servers", "1024 servers", "4096 servers", "512 servers"],
    correct: 0,
    explanation: `Step-by-step solution:

With 2k-port switches:
- Each leaf switch has 2k ports: k ports connect to servers, k ports connect to spine switches
- Number of spine switches = k
- Number of leaf switches = 2k (since each spine has 2k ports and connects to all leaves)
- Total servers = leaf switches × servers per leaf = 2k × k = 2k²

For k = 32:
  Total servers = 2 × 32² = 2 × 1024 = 2048 servers

With 64-port switches (2k = 64, k = 32):
  64 leaf switches, 32 spine switches → 2048 servers at 10 Gbit/s each = 20.48 Tbit/s total bandwidth.`
  },
...
The parallel system (0.99) is MORE reliable than the most reliable component (0.9). Adding parallel redundancy greatly improves reliability.`
  },
  {
    category: "Building Infrastructure",
    type: "theory",
    question: "True or False: Cooling towers use cold water to remove heat and to function effectively.",
    options: ["True", "False"],
    correct: 0,
    explanation: "True. Cooling towers use evaporative cooling principles — water is cooled by partial evaporation as it flows through the tower. The cooled water is then circulated through heat exchangers in the data center to absorb heat from IT equipment, then returned to the cooling tower to be cooled again."
  },
  {
    category: "Building Infrastructure",
    type: "theory",
    question: "True or False: Controlling humidity levels in a datacenter is important for preventing equipment failures.",
    options: ["True", "False"],
    correct: 0,
    explanation: "True. Humidity control is critical: too LOW humidity causes electrostatic discharge (ESD) which can damage sensitive electronic components. Too HIGH humidity causes condensation on equipment, leading to short circuits and corrosion. Data centers typically maintain humidity between 40-60% relative humidity."
  },
  {
    category: "Dependability",
    type: "theory",
